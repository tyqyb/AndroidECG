/**
 * 连接蓝牙后跳转至Recdata页面用于展示数据，曲线绘图，绘制指标2
 **/
...
public class DrawLine2 extends View {
    ...
    private void updateDataRange(float value) {
        if (value > maxValue) maxValue = value * 1.1f; // 增加10%空间
        if (value < minValue) minValue = Math.max(0, value * 0.9f); // 确保最小值不为负
    ...
    private void drawGrid(Canvas canvas) {
        ...
        for (int i = 0; i <= gridLines; i++) {
            ...
            String label = String.format(Locale.getDefault(), "%.1f", value);
    ...
    private void drawTitleAndStats(Canvas canvas) {
        ...
        String stats = String.format("数据点数: %d | 范围: %.1f-%.1f",
                dataQueue.size(), minValue, maxValue);
